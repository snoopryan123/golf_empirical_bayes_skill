...
df_fg_1A = read_csv("data_field_goals.csv")
df_fg_1 = 
  df_fg_1A %>%
...
for (CHUNK_SIZE in num_szns_chunk) {
  ...
  print(paste0("CHUNK_SIZE = ", CHUNK_SIZE))
  
  ### bin kickers into groups of CHUNK_SIZE seasons
  chunks = split(SZNS, ceiling(seq_along(SZNS) / CHUNK_SIZE))
  chunks
  
  df_season_group_A = 
    df_fg_1 %>%
    distinct(season) %>%
    mutate(season_group = ceiling(row_number() / CHUNK_SIZE)) 
  df_season_group_A
  ...
  df_season_group_B =
    ...
    summarize(
      season_label = str_c(min(season), "–", max(season))
    ) %>%
    ungroup()
  df_season_group_B
  
  df_season_group_C = left_join(df_season_group_A, df_season_group_B)
  df_season_group_C
  
  df_fg_2 = 
    df_fg_1 %>%
    left_join(df_season_group_C) %>%
    select(-season, -season_group) %>%
    group_by(kicker_id, season_label) %>%
    mutate(Player_num = cur_group_id(), N = n()) %>%
    ungroup() %>%
    arrange(Player_num) %>%
    mutate(Player = paste0(season_label," ",kicker_name)) %>%
    mutate(CHUNK_SIZE=CHUNK_SIZE)
  df_fg_2
  
  M = 25 # min num field goal attempts for each player-group
  table(df_fg_2$N)
  ...
  df_info = tibble(
    ...
    min_fg_attempts = M,
    num_unique_kickerGrps = length(unique(df0$Player_num)),
  )
  df_info
  gtsave(gt(df_info), paste0("results_fg_C",CHUNK_SIZE,"_plot_info.png"))

###############
### Results ###
###############

alphas = c(0.01, 0.05, 0.10, 0.15) 
...
# gt_effect_sizes
gtsave(gt_effect_sizes, paste0("results_fg_C",CHUNK_SIZE,"_plot_effectSizes.png"))
...
plot_EB_estimates = 
  ...
  ylab("Count") + theme(plot.margin = margin(r = 10,t=5,l=5,b=5) )
# plot_EB_estimates
ggsave(paste0("results_fg_C",CHUNK_SIZE,"_plot_EB.png"), plot_EB_estimates, width=6, height=4)
...
# plot_EB_shrinkage
ggsave(paste0("results_fg_C",CHUNK_SIZE,"_plot_EB_shrinkage.png"), plot_EB_shrinkage, width=7, height=5)
...
# plot_BH
ggsave(paste0("results_fg_C",CHUNK_SIZE,"_plot_BH.png"), plot_BH, width=6, height=3)
...
# gt_topkickers
gtsave(gt_topkickers, paste0("results_fg_C",CHUNK_SIZE,"_plot_topkickers.png"))
